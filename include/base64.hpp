 ...
 */
std::string base64Encode(uint8_t* buf_ptr, size_t buf_size)
{
 ...
 */
uint8_t * base64Decode(const std::string& text, uint8_t * buf_ptr = NULL )
{
	...
	if (!buf_ptr) {
		buf_ptr = new uint8_t[base64DecodedSize(text.size())];
	}
